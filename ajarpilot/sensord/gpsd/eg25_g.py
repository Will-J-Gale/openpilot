...
class GPS:
    def __init__(self):
        self.unpack_gps_meas, self.size_gps_meas = dict_unpacker(gps_measurement_report, True)
        self.unpack_gps_meas_sv, self.size_gps_meas_sv = dict_unpacker(gps_measurement_report_sv, True)

        self.unpack_glonass_meas, self.size_glonass_meas = dict_unpacker(glonass_measurement_report, True)
        self.unpack_glonass_meas_sv, self.size_glonass_meas_sv = dict_unpacker(glonass_measurement_report_sv, True)

        self.unpack_oemdre_meas, self.size_oemdre_meas = dict_unpacker(oemdre_measurement_report, True)
        self.unpack_oemdre_meas_sv, self.size_oemdre_meas_sv = dict_unpacker(oemdre_measurement_report_sv, True)

        self.unpack_svpoly, _ = dict_unpacker(oemdre_svpoly_report, True)
        self.unpack_position, _ = dict_unpacker(position_report)
    ...
    def read(self):
        ...
        if log_type == LOG_GNSS_OEMDRE_MEASUREMENT_REPORT:
            ...
            dat = self.unpack_oemdre_meas(log_payload)
            ...
            sats = log_payload[self.size_oemdre_meas:]
            for i in range(dat['svCount']):
                sat = self.unpack_oemdre_meas_sv(sats[self.size_oemdre_meas_sv*i:self.size_oemdre_meas_sv*(i+1)])
                sv = report.sv[i]
            ...
            msg = messaging.new_message('qcomGnss')
            dat = self.unpack_svpoly(log_payload)
            dat = relist(dat)
            gnss = msg.qcomGnss
            gnss.logTs = log_time
            gnss.init('drSvPoly')
            ...
            if log_type == LOG_GNSS_GPS_MEASUREMENT_REPORT:
                dat = self.unpack_gps_meas(log_payload)
                sats = log_payload[self.size_gps_meas:]
                unpack_meas_sv, size_meas_sv = self.unpack_gps_meas_sv, self.size_gps_meas_sv
                report.source = 0  # gps
                ...
                dat = self.unpack_glonass_meas(log_payload)
                sats = log_payload[self.size_glonass_meas:]
                unpack_meas_sv, size_meas_sv = self.unpack_glonass_meas_sv, self.size_glonass_meas_sv
